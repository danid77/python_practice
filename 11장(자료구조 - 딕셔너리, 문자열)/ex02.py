...
# 키값만 출력되도록 루프를 돌게한다
# 아래 print() 문을 이용하여 keys()를 사용하면 반환값의 형태가 dict_keys([])으로 출력된다.
# 타입은 "dict_keys"이다.
print(scores.keys())    # dict_keys(['국어', '수학', '영어'])
...
# 값만 출력하는 방법
# dict_values([]) 형태로 출력된다. 타입은 "dict_values"이다.
print(scores.values())
...
# 키와 값을 동시에 출력하는 방법
# dict_items([]) 형태로 출력된다. 타입은 "dict_items"이다.
print(scores.items())
...
# 딕셔너리를 리스트로 변환함
li1 = list(dic1)
li2 = list(dic1.values())   # 값만 리스트에 저장함
