...
set2 = { "안녕", "하이", "사람", "자동차" }
print("set2 : ",set2)
# 세트는 시퀀스형 자료형이기 때문에 일반적인 함수를 사용할 수 있다.
print(len(set1))
print(len(set2))

# 세트는 중복을 허용하지 않는 특징이 있으므로 중복된 값을 출력되지 않는다.
# 중복된 값은 제거되어 출력된다.
...
# set3[0] = 80
# print(set3[0])
# 'set' object does not support item assignment 에러 발생

print("---------------------------------------")
# 비어있는 세트를 만드는 방법은 {} 를 사용하지 아니하고 set() 호출하면 된다.
set4 = set()
# 세트에 값을 추가하기(add()사용)
# 세트는 모든 요소들을 해싱기법을 이용하여 저장하고 관리한다.그래서 요소들은 해싱(대표적인 : hashable)이 가능해야 한다.
...
# 세트는 요소가 해싱이 가능하려면 해쉬 코드를 가져야 한다고 하였다.
# 그 값이 변경되면 안된다.따라서 세트는 변경 가능한 항목을 가지면 안된다.
# 그래서 아래와 같이 리스트를 요소로 가지지 못하고 에러를 발생시킨다.
# set5 = {1.1, 2, "자동차", [10,20]} unhashable type: 'list'
