...
# 아래는 내장함수를 이용해서 최소값 최대값을 구하는 코드
num = [1,5,-9,100]
print("최소값 : ", min(num))
print("최대값 : ", max(num))

# 최대값돠 최소값을 구하는 알고리즘은 상당히 중요한 개념이므로 아래의 코드를 이해하도록 하자
prices = [1000, 3000, 500, 10000, 20000, 700]
# 먼저 prices[]에 있는 0번쨰 인덱스 값을 변수에 저장을 한다.
lowPrice = prices[0]
# 이후, 루프를 돌면서 조건식으로 값이 작으면 해당하는 값을 lowPrice 변수에 ㅡ재저ㅔ장
for i in range(1, len(prices)):
    # 참이라는 것은 prices[i]가 lowPrice 보다 작다라는 의미
...
words = ["dog", "cat","hores","lion","tiger","elephant"]
words_han = ["안녕", "하이","반갑습니다","가세요","오랜만입니다.","가지마세요"]
# 문자열 리스트에서 min()는 제일 첫 글자가 아스키코드 중에서 가장 작은 단어를 반환해준다.
# 문자열 리스트에서 max()는 제일 첫 글자가 아스키코드 중에서 가장 큰 단어를 반환해준다.
